...
<!-- workspace-kit-source: .workspace-kit/packages/skills/pr-review.SKILL.md -->
...
- **`effective_jira_project_key`** (optional ticket extraction hint), in this order:
  1. Explicit user-provided project key (if runner supports it).
  2. `AGENTS.md` Default Jira Project Key (if present).
  3. Environment: `DEFAULT_JIRA_PROJECT_KEY` or `JIRA_PROJECT_KEY`.
  4. Extract from changed artifact paths (`artifacts/tickets/KEY-123/...`).
  5. Extract from branch/title/body ticket tokens (`KEY-123`).
  6. Final fallback: unset (`null`) and continue without Jira enrichment.
...
### Optional Jira Connectivity

If Atlassian MCP is available, use it to enrich ticket acceptance checks. If unavailable, continue
with local artifacts and PR evidence.
...
3. If none found: fallback to ticket extraction from branch/title/body (`[PROJECT]-[N]`) and map to
   `artifacts/tickets/{ID}` (or slug variant if unique), preferring `effective_jira_project_key`
   when available.
...
- **Ticket completeness**: acceptance criteria and DoD intent are satisfied (from `ticket.md/spec.md/plan.md`,
  and Jira only when available).
- **Architecture and scope**: changes are in the correct layer/files and avoid unrelated scope creep.
...
1. **Cross-file consistency**: when one changed file defines or changes a data shape, API,
...
- Review Context Intake: github-pr|local-branch-diff
...
- **Jira unavailable/inaccessible**: continue with artifact-first completeness checks and warning.
