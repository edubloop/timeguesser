...
<!-- workspace-kit-source: .workspace-kit/packages/skills/implement.SKILL.md -->
...
When implementation is executed through Fabro stages, align behavior with
`.workspace-kit/templates/fabro/prompts/implement.md` and
`.workspace-kit/templates/fabro/prompts/fixup.md`, including prompt-hardening boundaries in
`.workspace-notes/prompt-hardening-pattern.md`.
...
3. Run the full final validation section (tests, typecheck/lint if applicable)
4. **All validation commands must pass.** If any returns non-zero, do NOT mark the plan
   Completed. Triage and fix, or stop and report the blocker.
5. Mark plan status as `Completed` and update `Last Updated` only after step 4 passes
...
- [ ] Every data-bound element renders real data (not placeholders or stubs)
