...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/run_fabro_delivery_approved.sh <TICKET_ID> <GOAL_FILE> [fabro args...]

Examples:
  ./scripts/run_fabro_delivery_approved.sh TG-101 ../artifacts/tickets/TG-101/ticket.md --preflight
  ./scripts/run_fabro_delivery_approved.sh TG-101 ../artifacts/tickets/TG-101/ticket.md --dry-run
  ./scripts/run_fabro_delivery_approved.sh TG-101 ../artifacts/tickets/TG-101/ticket.md
EOF
}
...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/timeguesser-fabro-approved-${ticket_id}.XXXXXX")"
...
cat >"$tmp_toml" <<EOF
version = 1
graph = "$workflow_dir/approved-workflow.fabro"
goal = "Run Approved Delivery mode for ticket $ticket_id in TimeGuesser"
directory = "$repo_root"
