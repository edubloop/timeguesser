...
- Write primarily to `$artifact_dir/shape.md`.
...
- Add a concise `Reviewer decision response` section to `shape.md` that maps each required change to the revision made.
- If a required change conflicts with an existing repo policy or ticket constraint, state that conflict explicitly in `shape.md` rather than silently reinterpreting the reviewer decision.
